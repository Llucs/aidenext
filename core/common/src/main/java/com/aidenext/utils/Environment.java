...
public final class Environment {
  ...
  public static void init(Context context) {
    ...
    trySetExecutable(JAVA);
    trySetExecutable(BASH_SHELL);

    try {
      System.setProperty("user.home", HOME.getAbsolutePath());
    } catch (Exception e) {
      LOG.error("Unable to set user.home property", e);
    }
  }

  public static void trySetExecutable(@NonNull final File file) {
    if (!file.exists()) {
      LOG.warn("Cannot set executable on non-existent file: {}", file);
      return;
    }
    setExecutable(file);
  }
